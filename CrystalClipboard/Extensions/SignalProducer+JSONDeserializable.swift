...
extension SignalProducerProtocol where Value == Any, Error == MoyaError {
    ...
    private func performDeserialization<Z>(_ deserialization: @escaping (Any) throws -> Z) -> SignalProducer<Z, MoyaError> {
        return producer.flatMap(.latest) { any -> SignalProducer<Z, MoyaError> in
            do {
                return SignalProducer(value: try deserialization(any))
            } catch {
